...
// ... (Helpers: computeIsLoggedIn, getStableProfileId, formatTextForDisplay - KEEP THESE AS IS) ...
...
function getStableProfileId(user) {
  if (user?.role === "admin") {
    return user?.userProfileID || user?.userID || user?.id || "admin-fallback";
  }
...
const formatTextForDisplay = (text) => {
  ...
  lines.forEach((line) => {
    ...
    if (boldMatch) {
      html += `<div class="rdp-section-header">${boldMatch[1]}</div>`;
      inList = false; return;
    }
    const numberedMatch = trimmedLine.match(/^(\d+)\.\s*(.*)/);
    if (numberedMatch) {
      html += `<div class="rdp-step"><span class="rdp-step-number">${numberedMatch[1]}.</span><span class="rdp-step-text">${numberedMatch[2]}</span></div>`;
      inList = false; return;
    }
    const bulletMatch = trimmedLine.match(/^[-•*]\s+(.*)/);
    if (bulletMatch) {
      html += `<div class="rdp-bullet-item">${bulletMatch[1]}</div>`;
      inList = true; return;
    }
    ...
    if (trimmedLine) {
      if (inList && !trimmedLine.match(/^[-•*]/)) inList = false;
...
// ... (LikeButton Component - KEEP AS IS) ...
...
// ... (CommentSection Component - KEEP AS IS) ...
const CommentSection = ({ postId, user, comments, onCommentAdded, onCommentDeleted, onAlert }) => {
  ...
  const canDeleteComment = (commentUserProfileID) => {
    ...
    const isOwner = parseInt(currentUserProfileID) === parseInt(commentUserProfileID);
    ...
    <div className="comment-section">
      ...
      {showDeleteModal && (
        <div className="modal-overlay">
          <div className="modal-card">
            <div className="modal-card-header"><h3>Delete Comment?</h3></div>
            <div className="modal-card-body"><p>This action cannot be undone.</p></div>
            <div className="modal-card-actions">
              <button className="lrp-btn lrp-btn-outline" onClick={() => setShowDeleteModal(false)}>Cancel</button>
              <button className="lrp-btn lrp-btn-danger" onClick={confirmDelete} disabled={deletingCommentId}>Delete</button>
            </div>
          </div>
        </div>
      )}
      <form className="comment-form" onSubmit={handleSubmit}>
        <textarea
          ...
          rows="3"
          placeholder={isGuest ? "Please log in to comment" : "Add a comment..."}
          value={comment}
...
export default function CommunityPost() {
  ...
  if (loading) return <><Header /><div className="community-page" style={{marginTop:"100px"}}><div className="loading">Loading...</div></div><Footer /></>;
  if (error) return <><Header /><div className="community-page" style={{marginTop:"100px"}}><div className="error"><h2>Error</h2><p>{error}</p></div></div><Footer /></>;
  if (!post) return <><Header /><div className="community-page" style={{marginTop:"100px"}}><div className="not-found"><h2>Post not found</h2></div></div><Footer /></>;

  return (
    <>
    <Header />
    
    {/* ✅ 1. ADDED marginTop: "100px" to force content down below the header */}
    <div className="community-page" style={{ marginTop: "100px" }}>
      
      {/* ✅ 2. MOVED BUTTON OUTSIDE the grid layout so it sits on top */}
