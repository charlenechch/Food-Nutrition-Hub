...
const LikeButton = ({ postId, initialLikes, user, onAlert }) => {
  ...
  return (
    <>
      ...
      <div className={`like-button ${isLiked ? 'is-liked' : ''}`} onClick={handleLike}>
        <div className="like">
          ...
          <span className="like-text">{likes === 1 ? t("communityPost.like") : t("communityPost.likes")}</span>
        </div>
        <span className="like-count one">{isLiked ? likes : likes}</span>
        <span className="like-count two">{isLiked ? likes : likes}</span>
      </div>
    </>
  );
};
