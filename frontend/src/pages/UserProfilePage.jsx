// ✅ UserProfilePage.jsx – Final Clean Version (Supports /profile + /profile/:userProfileID)
// ✅ Keeps saved foods, contributions, preferences, settings, stats, and edit profile
// ✅ Automatically loads logged-in user using session; redirects guests to login
...
export default function UserProfilePage() {
  ...
  const savePersonal = async () => {
    try {
      if (!user?.userProfileID) return alert("No Profile ID found.");
      const res = await fetch(`${API_BASE_URL}/api/userProfile/${user.userProfileID}`, {
        method: "PUT",
  ...
  const savePrefs = async () => {
    try {
      if (!user?.userProfileID) return alert("No Profile ID found.");
      const res = await fetch(`${API_BASE_URL}/api/userProfile/${user.userProfileID}/preferences`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
    ...
    return (
      <div className="user-profile-page">
        ...
        <div className="upp-page">
          ...
          <p>We couldn’t load your profile. Try again from the Profile menu.</p>
          <div className="upp-error-actions">
            <button className="lrp-btn lrp-btn-outline" onClick={() => navigate("/")}>Go Home</button>
          </div>
        </div>
