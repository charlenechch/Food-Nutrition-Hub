  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.foodName || !formData.culturalOrigin || !formData.culturalStory || !selectedFile) {
      setModal({
        ...
        message: "Please ensure Food Name, Origin, Story, and a Photo are provided.",
    ...
    try {
      ...
      // Ensure submitting is false before triggering modal to prevent freezing
      setSubmitting(false);
      ...
      if (response.ok && result.success) {
        setModal({
          ...
          message: "Your heritage story is pending admin approval. You can track it in your profile status.",
          icon: <PiChefHat />,
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="community-pagination">
                    <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1} className="community-page-btn nav-btn">← Prev</button>
                    <div className="page-numbers">
                      {[...Array(totalPages)].map((_, i) => (
                          <button key={i + 1} onClick={() => paginate(i + 1)} className={`community-page-btn page-num ${currentPage === i + 1 ? "active" : ""}`}>{i + 1}</button>
                      ))}
                    </div>
                    <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages} className="community-page-btn nav-btn">Next →</button>
                </div>
              )}
            </>
          )}
